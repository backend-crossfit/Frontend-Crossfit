...
async function getUsuario() {
    try {
        ...
        if (useUsuario.estatus === 200) {
            usuarios.value = response.filter(us => us.rol === 'Administrador').map(pe => ({
...
function irEditarAdmin(usuario) {
    const url = router.resolve({ path: '/editar-admin', query: { id: usuario._id } }).href;
                ...
                <table class="table table-dark table-striped table-hover">
                    <thead>
                        <tr style="text-align: center;">
                            ...
                            <th scope="col" colspan="3" style="color: #fdfefe;">Acciones</th>
                        </tr>
                    </thead>
                    <tbody>
